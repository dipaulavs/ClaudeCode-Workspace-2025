...
Marca cliente como interessado em carro/imóvel específico:
1. Cria tag no Chatwoot (ex: "interessado_gol_2020" ou "interessado_imovel_001")
2. Salva carro/imóvel ativo no Redis

FUNÇÕES DISPONÍVEIS:
- taguear_cliente(numero_cliente, carro_id, redis_client, chatwoot_config)
- obter_carro_ativo(numero_cliente, redis_client)
- obter_imovel_ativo(numero_cliente, redis_client) [alias]
- limpar_carro_ativo(numero_cliente, redis_client)
- limpar_imovel_ativo(numero_cliente, redis_client) [alias]
"""
...
def taguear_cliente(
    ...
    """
    Tageia cliente no Chatwoot e marca carro/imóvel ativo no Redis

    Args:
        numero_cliente: Número do cliente (ex: "5531986549366")
        carro_id: ID do carro/imóvel (ex: "gol-2020-001" ou "imovel-001")
...
def obter_imovel_ativo(numero_cliente: str, redis_client) -> Optional[str]:
    """
    Obtém imóvel ativo do cliente no Redis
    Alias para obter_carro_ativo adaptado para imóveis
    ...
    Returns:
        imovel_id ou None
    """
    return obter_carro_ativo(numero_cliente, redis_client)
...
def limpar_imovel_ativo(numero_cliente: str, redis_client) -> bool:
    """
    Limpa imóvel ativo do cliente (útil para reset)
    Alias para limpar_carro_ativo adaptado para imóveis
    ...
    Returns:
        bool (sucesso)
    """
    return limpar_carro_ativo(numero_cliente, redis_client)
