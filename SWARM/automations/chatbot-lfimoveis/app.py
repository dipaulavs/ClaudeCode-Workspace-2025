#!/usr/bin/env python3
"""
🏠 CHATBOT LF IMÓVEIS V1 - FRAMEWORK HÍBRIDO COMPLETO
...
✅ Features principais:
  ...
  - Score automático (lead quente/morno/frio)
  ...
  - Métricas e relatórios (dashboard diário)
  - Orquestrador completo (pipeline único)

📍 Adaptado de: chatbot_automaia_v4.py (carros → imóveis)
"""
    ...
    try:
        ...
        payload = {
            ...
            "messages": [{
                "role": "user",
                "content": [
                    {"type": "text", "text": "Descreva esta imagem detalhadamente (2-3 frases). Se for um imóvel, mencione características."},
...
def enviar_imagem_whatsapp(phone, image_url, caption=""):
    ...
    try:
        ...
        if response.status_code in [200, 201]:
            print(f"✅ Imagem enviada!", flush=True)
            return True
        else:
...
def analisar_completude_mensagem(mensagens):
    ...
    texto_completo = "\n".join(mensagens)
    prompt = f"""Analise se esta mensagem está COMPLETA ou INCOMPLETA.

MENSAGEM: "{texto_completo}"

REGRAS:
- "..." → INCOMPLETA
- Muito curta ("oi") → INCOMPLETA
- Vírgula no final → INCOMPLETA
...
def processar_mensagens_agrupadas(numero):
    ...
    try:
        ...
        # Salva mensagens usuário
        for msg in mensagens:
            salvar_contexto(numero, msg, "user")

        # 🚀 ORQUESTRADOR
        print("🤖 Chamando Orquestrador...", flush=True)
        ...
        print(f"✅ Score: {score} ({classificacao})", flush=True)
        print(f"✅ Resposta: {resposta[:100]}...", flush=True)
        ...
        if fotos:
            print(f"📸 Enviando {len(fotos)} foto(s)...", flush=True)
            for foto in fotos:
                enviar_imagem_whatsapp(numero, foto)
                time.sleep(2)

            # Callback: fotos enviadas (agenda follow-up pós-fotos)
...
def webhook_chatwoot():
    ...
    try:
        ...
        print(f"\n{'='*80}", flush=True)
        print(f"🔔 WEBHOOK CHATWOOT → BOT LF IMÓVEIS V1 - {datetime.now().strftime('%H:%M:%S')}", flush=True)
...
def health():
    ...
    return jsonify({
        "status": "online",
        "version": "1.0 - FRAMEWORK HÍBRIDO COMPLETO",
        "model": "anthropic/claude-haiku-4.5",
        "redis": redis_status,
        ...
        "features": [
            ...
            "✅ 4 Ferramentas (lista_imoveis, consulta_faq, tagueamento, agendar_visita)",
            "✅ Score automático (lead quente/morno/frio)",
...
if __name__ == '__main__':
    print("=" * 70)
    print("🏠 CHATBOT LF IMÓVEIS V1 - FRAMEWORK HÍBRIDO COMPLETO!")
    print("=" * 70)
    print("✨ Componentes:")
    print("   ✅ Agente Principal + 4 Ferramentas")
    print("   ✅ Score (lead quente/morno/frio)")
    print("   ✅ Follow-ups automáticos")
    print("   ✅ Escalonamento inteligente")
    print("   ✅ Agendamento de visitas")
    print("   ✅ Métricas e relatórios")
    print()
    print(f"🌐 Webhook: http://localhost:5010/webhook/chatwoot")
    print(f"💚 Health: http://localhost:5010/health")
    print("=" * 70)
