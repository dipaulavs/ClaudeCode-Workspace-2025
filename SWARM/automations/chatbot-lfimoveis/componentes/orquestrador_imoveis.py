#!/usr/bin/env python3
"""
🎯 ORQUESTRADOR IMÓVEIS - Framework Híbrido Completo
...
# RAG Simples
try:
    from componentes.rag_simples_imoveis import RAGSimplesImoveis
    RAG_DISPONIVEL = True
except ImportError:
...
class OrquestradorImoveis:
    """
    Orquestrador adaptado para imóveis
    """
    ...
    def __init__(self, imoveis_dir, openai_api_key, openrouter_api_key, redis_client, config):
        ...
        print("\n🎯 Inicializando Orquestrador Imóveis...", flush=True)

        # RAG Simples
        if RAG_DISPONIVEL:
            try:
                self.rag = RAGSimplesImoveis(
                    imoveis_dir,
                    openai_api_key,
                    openrouter_api_key,
                    redis_client
        ...
        # Score (com análise IA habilitada por padrão)
        if SCORE_DISPONIVEL:
            try:
                # Score espera chatwoot_config (não config completo)
                # usar_ia=True por padrão (análise com Claude)
                # Desabilitar com usar_ia=False se preferir apenas keywords
                self.score = IntegradorScore(redis_client, config.get('chatwoot', config), usar_ia=True)
                print("✅ Score inicializado (com análise IA)", flush=True)
            except Exception as e:
        ...
        print("🎉 Orquestrador Imóveis pronto!\n", flush=True)
    ...
    def processar_mensagem(
        ...
        """
        print(f"\n{'='*80}", flush=True)
        print(f"🎯 ORQUESTRADOR IMÓVEIS - {datetime.now().strftime('%H:%M:%S')}", flush=True)
            ...
            try:
                ...
                if "[ENVIAR_FOTOS:" in resposta_rag:
                    ...
                    if match:
                        ...
                        from ferramentas.tagueamento import obter_imovel_ativo
                        imovel_path = Path(__file__).parent.parent / "imoveis" / imovel_id
