...
class SistemaFollowUp:
    ...
    def __init__(self):
        ...
        # Carregar mensagens do arquivo editável
        self.triggers = self._carregar_mensagens()

    def _carregar_mensagens(self) -> Dict:
        """
        Carrega mensagens do arquivo editável.

        Returns:
            Dict com configuração de triggers (usa TRIGGERS hardcoded como fallback)
        """
        # Caminho do arquivo (3 níveis acima: componentes/followup/ -> raiz)
        arquivo = Path(__file__).parent.parent.parent / "mensagens_followup.txt"

        if not arquivo.exists():
            print("⚠️ Arquivo mensagens_followup.txt não encontrado. Usando mensagens padrão.")
            return TRIGGERS
        ...
        try:
            with open(arquivo, 'r', encoding='utf-8') as f:
                for linha in f:
                    ...
                    # Detectar tag (ex: [INATIVIDADE_2H])
                    if linha.startswith('[') and linha.endswith(']'):
                        tag_nome = linha[1:-1]  # Remove []

                        # Mapear tag do arquivo para nome do trigger no código
                        mapeamento = {
                            'INATIVIDADE_2H': 'inatividade_2h',
                            'INATIVIDADE_24H': 'inatividade_24h',
                            'INATIVIDADE_48H': 'inatividade_48h',
                            'POS_FOTOS': 'pos_fotos',
                            'POS_VISITA': 'pos_visita',
                            'LEMBRETE_24H_ANTES': 'lembrete_visita_24h',
                            'LEMBRETE_2H_ANTES': 'lembrete_visita_2h'
                        }

                        if tag_nome in mapeamento:
                            tag_atual = mapeamento[tag_nome]
                        else:
                            tag_atual = None

                    # Capturar mensagem (próxima linha não-vazia após tag)
                    elif tag_atual and linha and not linha.startswith('═'):
                        mensagens_customizadas[tag_atual] = linha
                        tag_atual = None  # Resetar para próxima tag

            # Mesclar mensagens customizadas com TRIGGERS padrão
            triggers_atualizados = TRIGGERS.copy()
            for trigger, mensagem in mensagens_customizadas.items():
                if trigger in triggers_atualizados:
                    triggers_atualizados[trigger]['mensagem'] = mensagem

            print(f"✅ {len(mensagens_customizadas)} mensagens customizadas carregadas")
            return triggers_atualizados

        except Exception as e:
            print(f"❌ Erro ao carregar mensagens: {e}. Usando mensagens padrão.")
            return TRIGGERS
    ...
    def registrar_envio(self, cliente_numero: str, trigger: str, tipo: str):
        ...
        self.redis_client.lpush(historico_key, json.dumps(historico_item))
    ...
    def registrar_resposta(self, cliente_numero: str):
        ...
        if historico:
            ultimo_item = json.loads(historico[0])
            ultimo_item["respondeu"] = True

            # Atualizar
            self.redis_client.lset(historico_key, 0, json.dumps(ultimo_item))

            # Incrementar métricas
            self._incrementar_metricas(ultimo_item["trigger"], "respondidos")

            print(f"✅ Resposta registrada: {cliente_numero} → {ultimo_item['trigger']}")
