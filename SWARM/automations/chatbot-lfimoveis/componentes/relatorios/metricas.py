...
class ColetorMetricas:
    ...
    def adicionar_lista(self, metrica: str, item: str, data: Optional[date] = None) -> None:
        """
        Adiciona item em lista de métrica

        Args:
            metrica: Nome da métrica (ex: "leads_quentes")
            item: Item a adicionar (ex: número de telefone)
            data: Data da métrica (padrão: hoje)
        """
        if data is None:
            data = datetime.now().date()
        ...
        # Evita duplicatas
        if not self.redis.lpos(chave, item):
            self.redis.lpush(chave, item)

        # Define expiração de 90 dias
        self.redis.expire(chave, 90 * 24 * 60 * 60)

    def incrementar_sorted_set(self, metrica: str, item: str, score: int = 1, data: Optional[date] = None) -> None:
        """
        Incrementa score em sorted set

        Args:
            metrica: Nome da métrica (ex: "imoveis_mais_procurados")
            item: Item a incrementar (ex: imovel_id)
        ...
        chave = self._gerar_chave(metrica, data)
        self.redis.zincrby(chave, score, item)
