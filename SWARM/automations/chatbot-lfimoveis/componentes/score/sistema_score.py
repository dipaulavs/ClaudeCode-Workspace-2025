...
class SistemaScore:
    ...
    - Intenção de Compra (max 50): quer visitar (+40), quer fechar (+50) - LEAD QUENTE!
    - Urgência (max 20): urgente, esta semana, este mês

    Score >= 70: QUENTE | Score 40-69: MORNO | Score < 40: FRIO
    """
    ...
    PESOS = {
        ...
        # Intenção de Compra (max 50)
        "quer_visitar": 40,            # "quero agendar", "quero visitar"
        "quer_fechar": 50,             # "fechar negócio", "quer proposta"
    ...
    KEYWORDS = {
        ...
        "quer_visitar": ["quero agendar", "quero visitar", "agendar visita", "marcar visita", "quero ver", "quer agendar"],
        "quer_fechar": ["fechar negócio", "fechar negocio", "quero proposta", "proposta", "quer fechar", "vamos fechar"],
    ...
    def calcular_delta(self, mensagem: str, estado_cliente: Dict[str, Any]) -> int:
        ...
        # Intenção de Compra - LEAD QUENTE
        if self._detectar_palavras(mensagem_lower, self.KEYWORDS["quer_visitar"]):
            if not estado_cliente.get("quer_visitar"):
                delta += self.PESOS["quer_visitar"]
                estado_cliente["quer_visitar"] = True

        if self._detectar_palavras(mensagem_lower, self.KEYWORDS["quer_fechar"]):
            if not estado_cliente.get("quer_fechar"):
                delta += self.PESOS["quer_fechar"]
                estado_cliente["quer_fechar"] = True
