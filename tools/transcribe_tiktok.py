#!/usr/bin/env python3
"""
Transcritor de Vídeos do TikTok via Apify + OpenAI
Transcreve automaticamente o áudio de vídeos do TikTok
"""
...
ACTOR_ID = "Tp2ubgGvSUJAW1cdg"
APIFY_API_URL = "https://api.apify.com/v2"
DOWNLOADS_DIR = Path.home() / "Downloads"

def run_transcription_actor(tiktok_url, model="whisper-1", response_format="json"):
    """
    Executa o actor do Apify para transcrever vídeos do TikTok
    """
    ...
    print(f"🎵 Iniciando transcrição do TikTok...")
...
def save_transcription(tiktok_url, transcription_data, model):
    """
    Salva a transcrição em arquivo
    """
    # Cria pasta com timestamp
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = DOWNLOADS_DIR / f"tiktok_transcription_{timestamp}"
    ...
    with open(transcription_file, 'w', encoding='utf-8') as f:
        f.write(f"TRANSCRIÇÃO DE VÍDEO DO TIKTOK\n")
        f.write(f"=" * 60 + "\n\n")
        f.write(f"URL: {tiktok_url}\n")
...
def transcribe_tiktok(tiktok_url, model="whisper-1"):
    """
    Transcreve um vídeo do TikTok
    """
    # Executa o Actor
    dataset_id = run_transcription_actor(tiktok_url, model)
    ...
    # Salva transcrição
    save_transcription(tiktok_url, results, model)

def main():
    parser = argparse.ArgumentParser(
        description="Transcreve áudio de vídeos do TikTok automaticamente",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos:
  # Transcrever um vídeo do TikTok
  python3 transcribe_tiktok.py "https://vm.tiktok.com/ZMACh1Mje/"

  # Transcrever URL completa
  python3 transcribe_tiktok.py "https://www.tiktok.com/@user/video/123456"

  # Usar modelo específico
  python3 transcribe_tiktok.py "URL" --model whisper-1

Modelos disponíveis:
  - whisper-1 (padrão, OpenAI Whisper)
  - gpt-4o-mini-transcribe (alternativo)
        """
    )

    parser.add_argument(
        "url",
        help="URL do vídeo do TikTok"
    )

    parser.add_argument(
        "--model",
        default="whisper-1",
        help="Modelo de transcrição OpenAI (padrão: whisper-1)"
    )

    args = parser.parse_args()

    transcribe_tiktok(args.url, args.model)
