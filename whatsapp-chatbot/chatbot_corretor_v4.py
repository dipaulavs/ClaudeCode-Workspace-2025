#!/usr/bin/env python3
"""
🤖 CHATBOT CORRETOR V4.2 - VERSÃO COMPLETA COM VISÃO
...
# 🏠 Dados dos imóveis (carregados ao iniciar)
...
def carregar_imoveis():
    ...
    Returns:
        dict: Dicionário com dados dos imóveis {id: {...}}
    """
    if not IMOVEIS_DIR.exists():
        print("⚠️  Diretório de imóveis não encontrado. Criando...", flush=True)
    ...
    for imovel_dir in IMOVEIS_DIR.iterdir():
        ...
        try:
            ...
            print(f"✅ Imóvel carregado: {imovel_id} ({len(imovel_data['fotos'])} fotos)", flush=True)

        except Exception as e:
            print(f"⚠️  Erro ao carregar {imovel_id}: {e}", flush=True)

    return imoveis

def buscar_imovel_por_contexto(mensagem):
    """
    🔍 Busca imóvel relevante baseado na mensagem do usuário

    Returns:
        dict or None: Dados do imóvel encontrado ou None
    """
    ...
    # Se tem só um imóvel, retorna ele
    if len(imoveis_database) == 1:
        return list(imoveis_database.values())[0]

    # Busca por palavras-chave na descrição
    palavras_busca = ["apartamento", "casa", "imóvel", "quarto", "sala", "vaga", "garagem", "foto", "imagem"]

    for palavra in palavras_busca:
        if palavra in mensagem_lower:
            # Retorna o primeiro imóvel (pode ser melhorado com busca semântica)
            if imoveis_database:
                return list(imoveis_database.values())[0]
...
# Prompt do corretor
PROMPT_CORRETOR_BASE = """Vc é Ricardo, corretor de imóveis descontraído e gente boa.
...
- Emojis sempre!
- Quando o cliente pedir fotos ou informações de imóveis, SEMPRE responda: "[ENVIAR_FOTOS:ID_DO_IMOVEL]" no final da mensagem
- Use esse formato EXATAMENTE assim para que o sistema envie as fotos automaticamente"""

def gerar_prompt_com_imoveis():
    """Gera prompt incluindo informações dos imóveis disponíveis"""
    prompt = PROMPT_CORRETOR_BASE

    if imoveis_database:
        prompt += "\n\nIMÓVEIS DISPONÍVEIS:\n"
        for imovel_id, dados in imoveis_database.items():
            prompt += f"\n• ID: {imovel_id}"
            if dados['descricao']:
                # Pega só as primeiras linhas da descrição
                desc_resumo = '\n'.join(dados['descricao'].split('\n')[:5])
                prompt += f"\n{desc_resumo}"
            if dados['fotos']:
...
Responda APENAS "COMPLETA" ou "INCOMPLETA" (uma palavra só!)"""
    ...
    try:
        ...
        headers = {
            ...
            "X-Title": "Chatbot Corretor - Analisador"
        }
...
def obter_contexto_historico(numero):
    ...
    try:
        chave_contexto = f"contexto:{numero}"
...
def gerar_resposta_openrouter(mensagem_usuario):
    """Gera resposta usando OpenRouter com contexto dos imóveis"""
    url = "https://openrouter.ai/api/v1/chat/completions"
    ...
    headers = {
        ...
        "Content-Type": "application/json",
        "HTTP-Referer": "https://github.com/felipemdepaula/claude-code-workspace",
        "X-Title": "Chatbot Corretor WhatsApp V4"
    }

    # Usa prompt com informações dos imóveis
    prompt_sistema = gerar_prompt_com_imoveis()
...
def processar_mensagens_agrupadas(numero):
    ...
    try:
        ...
        if "[ENVIAR_FOTOS:" in resposta:
            ...
            for imovel_id in matches:
                imovel_id = imovel_id.strip()

                if imovel_id in imoveis_database:
                    imovel = imoveis_database[imovel_id]

                    print(f"📸 Comando detectado! Enviando fotos do {imovel_id}...", flush=True)

                    # Remove o comando da resposta
                    resposta = resposta.replace(f"[ENVIAR_FOTOS:{imovel_id}]", "").strip()
                    ...
                    # Envia todas as fotos do imóvel
                    if imovel['fotos']:
                        total_fotos = min(len(imovel['fotos']), 5)
                        print(f"📤 Enviando {total_fotos} foto(s)...", flush=True)

                        fotos_sucesso = 0
                        for i, foto in enumerate(imovel['fotos'][:5], 1):  # Máximo 5 fotos
                            link = foto.get('link')
                            nome = foto.get('nome', '')
                        ...
                        enviar_resposta_whatsapp(numero, "Ops, ainda não tenho fotos desse imóvel 😅")
...
def webhook_chatwoot():
    ...
    ✅ V4: Adiciona na fila com debounce inteligente
    ...
    try:
        ...
        print(f"\n{'='*80}", flush=True)
        print(f"🔔 WEBHOOK CHATWOOT → BOT V4 - {datetime.now().strftime('%H:%M:%S')}", flush=True)
...
def health():
    ...
    total_fotos = sum(len(i['fotos']) for i in imoveis_database.values())
    ...
    return jsonify({
        ...
        "redis": redis_status,
        "imoveis": {
            "total": len(imoveis_database),
            "total_fotos": total_fotos,
            "ids": list(imoveis_database.keys())
        },
        "features": [
...
if __name__ == '__main__':
    print("=" * 70)
    print("🤖 CHATBOT CORRETOR V4.3 - VERSÃO COMPLETA COM IMÓVEIS!")
    print("=" * 70)
    ...
    print("   🏠 Banco de dados de imóveis")
    print("   📸 Envio automático de fotos")
    print()
    print(f"🌐 Webhook: http://localhost:5001/webhook/chatwoot")
    print(f"💚 Health: http://localhost:5001/health")
    print("=" * 70)

    # Carrega imóveis
    print("\n🏠 Carregando imóveis...")
    imoveis_database.update(carregar_imoveis())

    if imoveis_database:
        print(f"✅ {len(imoveis_database)} imóvel(is) carregado(s)!")
        total_fotos = sum(len(i['fotos']) for i in imoveis_database.values())
        print(f"📸 Total de fotos: {total_fotos}")
    else:
        print("⚠️  Nenhum imóvel encontrado.")
        print(f"💡 Dica: Use 'python3 upload_fotos_imoveis.py' para adicionar imóveis")
