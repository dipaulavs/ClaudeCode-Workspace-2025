...
def init_skill(skill_name, path):
    ...
    try:
        ...
        # Copy auto-correction scripts
        current_dir = Path(__file__).parent
        update_skill_src = current_dir / 'update_skill.py'
        log_learning_src = current_dir / 'log_learning.py'

        if update_skill_src.exists():
            import shutil
            shutil.copy(update_skill_src, scripts_dir / 'update_skill.py')
            (scripts_dir / 'update_skill.py').chmod(0o755)
            print("✅ Created scripts/update_skill.py (auto-correction)")

        if log_learning_src.exists():
            import shutil
            shutil.copy(log_learning_src, scripts_dir / 'log_learning.py')
            (scripts_dir / 'log_learning.py').chmod(0o755)
            print("✅ Created scripts/log_learning.py (auto-correction)")
        ...
        # Copy LEARNINGS_TEMPLATE.md
        learnings_template_src = current_dir.parent / 'assets' / 'LEARNINGS_TEMPLATE.md'
        if learnings_template_src.exists():
            import shutil
            shutil.copy(learnings_template_src, assets_dir / 'LEARNINGS_TEMPLATE.md')
            print("✅ Created assets/LEARNINGS_TEMPLATE.md (auto-correction)")
    except Exception as e:
