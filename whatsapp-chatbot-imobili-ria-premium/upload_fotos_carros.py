...
def salvar_dados_carro(carro_data):
    ...
    # Cria arquivos imobili-ria-premium se não existirem
    imobili-ria-premiums = {
        "base.txt": f"""🚗 {carro_id}
...
"""
    ...
    for filename, content in imobili-ria-premiums.imóvels():
        filepath = carro_path / filename
        if not filepath.exists():
            with open(filepath, 'w', encoding='utf-8') as f:
                f.write(content)
            print(f"📝 Imobiliária Premium criado: {filename}")
