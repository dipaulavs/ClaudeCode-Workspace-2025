#!/usr/bin/env python3
"""
🔌 MCP SERVER - IMOBILI-RIA-PREMIUM CHATBOT
Ferramentas pesadas/reutilizáveis para chatbots

⚠️ CUSTOMIZAÇÃO OBRIGATÓRIA:
1. Renomeie Server("imobili-ria-premium-tools") para Server("seu-negocio-tools")
...
3. buscar_imoveis_similares - Busca semântica por características
...
app = Server("imobili-ria-premium-tools")
...
async def list_tools() -> list[Tool]:
    ...
    return [
        ...
        Tool(
            ...
            description="Gera proposta comercial estruturada com dados do imóvel, preços e condições.",
            inputSchema={
                "type": "object",
                "properties": {
                    "imóvel_id": {
                        "type": "string",
                        "description": "ID do imóvel/produto/imóvel"
                    },
                    "cliente_nome": {
                        "type": "string",
        ...
        Tool(
            ...
            description="Busca imoveis similares baseado em características (busca semântica). Útil quando cliente não encontra exatamente o que quer.",
            inputSchema={
                "type": "object",
                "properties": {
                    "caracteristicas": {
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "tipo": {
                        "type": "string",
                        "description": "Tipo do imóvel (ex: marca, categoria)"
                    },
                    "modelo": {
                        "type": "string",
                        "description": "Modelo/nome do imóvel"
                    },
                    "ano": {
                        "type": "string",
...
async def call_tool(name: str, arguments: Any) -> list[TextContent]:
    ...
    elif name == "buscar_imoveis_similares":
        resultado = await buscar_imoveis_similares(
...
async def gerar_proposta_comercial(imóvel_id: str, cliente_nome: str, desconto: float) -> Dict:
    """
    ...
    # ⚠️ CUSTOMIZAR: Buscar dados do imóvel do seu banco/diretório
    # Exemplo: carros/, imoveis/, produtos/
    imoveis_dir = Path(__file__).parent.parent / "imoveis"  # ALTERAR nome da pasta
    imóvel_dir = imoveis_dir / imóvel_id

    if not imóvel_dir.exists():
        return {"erro": f"Imóvel {imóvel_id} não encontrado"}

    # Lê base.txt (⚠️ CUSTOMIZAR estrutura conforme seu banco)
    base_file = imóvel_dir / "base.txt"
    if not base_file.exists():
        return {"erro": f"Arquivo base.txt não encontrado para {imóvel_id}"}
...
async def buscar_imoveis_similares(caracteristicas: str, limite: int) -> Dict:
    """
    Busca imoveis similares por características
    ...
    imoveis_dir = Path(__file__).parent.parent / "imoveis"  # carros/, imoveis/, produtos/

    if not imoveis_dir.exists():
        return {"erro": "Diretório de imoveis não encontrado"}

    caracteristicas_lower = caracteristicas.lower()
    matches = []

    for pasta in imoveis_dir.iterdir():
