# 🔌 IMOBILI-RIA-PREMIUM CHATBOT v2.0 - MCP Híbrido

Imobiliária Premium genérico para criar chatbots WhatsApp com arquitetura híbrida (Function Calling + MCP).
...
```python
# Linha 38
app = Server("imobili-ria-premium-tools")  # ← MUDAR para "seu-negocio-tools"
```
...
```python
# imoveis → carros/imoveis/produtos
# imóvel → carro/imovel/produto
```

### 3. Ferramentas Locais (`ferramentas/`)

**Adapte para seu negócio:**
- `lista_imoveis.py` → `lista_carros.py`
...
**O que ele faz:**
1. ✅ Valida que imobili-ria-premium tem estrutura MCP
2. ✅ Copia todo o imobili-ria-premium
3. ✅ Renomeia `imoveis/` → `carros/` (conforme nicho)
4. ✅ Renomeia `imobili-ria-premium-tools` → `seu-negocio-tools`
...
## 📁 Estrutura do Imobiliária Premium
...
- ✅ Substitui palavras-chave (imoveis → carros)
...
| Cenário | Tempo Médio |
|---------|-------------|
| Cliente lista imoveis | 1.4s (local) |
...
- ✅ Validação de imobili-ria-premium no script de criação
