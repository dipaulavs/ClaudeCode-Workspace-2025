...
class IntegradorFollowUp:
    ...
    def __init__(self, redis_client=None, config=None):
        ...
        Args:
            ...
            config: Config completo (compatibilidade)
        """
    ...
    def obter_historico(self, cliente_numero: str, limite: int = 10) -> list:
        ...
        historico = []
        for imóvel_json in historico_json:
            imóvel = json.loads(imóvel_json)
            imóvel["timestamp_formatado"] = datetime.fromtimestamp(
                imóvel["timestamp"]
            ).strftime("%d/%m/%Y %H:%M")
            historico.append(imóvel)
