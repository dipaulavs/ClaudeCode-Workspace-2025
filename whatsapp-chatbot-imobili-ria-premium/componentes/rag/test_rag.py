...
def teste_dois_estagios():
    ...
    # Cenário 1: Cliente novo → sem imóvel_ativo
    print("\n📋 Cenário 1: Cliente novo (ESTÁGIO 1)")
    print("-" * 50)

    imóvel_ativo = integrador._get_imóvel_ativo(cliente_teste)
    assert imóvel_ativo is None, "❌ Cliente novo não deve ter imóvel ativo"

    print("✅ Cliente novo → sem imóvel_ativo")

    # Cenário 2: Busca → define imóvel_ativo
    print("\n📋 Cenário 2: Após busca (define imóvel_ativo)")
    print("-" * 50)
    ...
    # Verifica se definiu imóvel_ativo
    imóvel_ativo = integrador._get_imóvel_ativo(cliente_teste)

    if imóvel_ativo:
        print(f"✅ Imóvel ativo definido: {imóvel_ativo}")
    else:
        print("⚠️  Múltiplos candidatos (lista apresentada)")

    # Cenário 3: Próxima pergunta usa imóvel_ativo (ESTÁGIO 2)
    if imóvel_ativo:
        print("\n📋 Cenário 3: Com imóvel_ativo (ESTÁGIO 2)")
        print("-" * 50)
