...
# Use lista_itens (genérico) OU lista_carros/lista_imoveis (específico)
try:
    from lista_itens import listar_itens_disponiveis, formatar_lista_para_mensagem
except ImportError:
    from lista_carros import listar_carros_disponiveis as listar_itens_disponiveis
    from lista_carros import formatar_lista_para_mensagem

from consulta_faq import consultar_faq_carro as consultar_faq_item
from tagueamento import obter_carro_ativo as obter_item_ativo, taguear_cliente
...
class RAGHibrido:
    ...
    def __init__(self, itens_dir, openai_api_key, openrouter_api_key, redis_client, mcp_server_path: Optional[str] = None):
        self.itens_dir = itens_dir
    ...
    async def _processar_mensagem_async(self, numero_cliente: str, mensagem: str, contexto: list) -> str:
        ...
        # Verifica item ativo
        item_ativo = obter_item_ativo(numero_cliente, self.redis)
...
1. **lista_itens** - Lista itens disponíveis
2. **consulta_faq** - Consulta FAQ do item ativo
3. **taguear_cliente** - Marca cliente interessado em item
...
7. **buscar_itens_similares** - Busca por características
8. **calcular_financiamento** - Simulação de financiamento
9. **consultar_tabela_preco** - Preço de mercado/tabela

{"🎯 ITEM ATIVO: " + item_ativo if item_ativo else "❌ SEM ITEM ATIVO"}

⚠️ QUANDO USAR CADA FERRAMENTA:

📋 **lista_itens**: Cliente pergunta "o que tem", "me mostra", "quais opções"
❓ **consulta_faq**: Cliente pergunta sobre item específico (preço, detalhes)
🏷️ **taguear_cliente**: Cliente demonstra interesse em item específico
...
🔍 **buscar_itens_similares**: Cliente não encontrou o que quer ("tem algo parecido?")
...
"""
        ...
        ferramentas_locais = [
            {
                ...
                "function": {
                    "name": "lista_itens",
                    "description": "Lista todos os itens disponíveis",  # ⚠️ CUSTOMIZAR
            ...
            },
            {
                ...
                "function": {
                    "name": "consulta_faq",
                    "description": "Consulta FAQ do item ativo",
                    "parameters": {
                        "type": "object",
                        "properties": {
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Marca cliente como interessado em item",
                    "parameters": {
                        "type": "object",
                        "properties": {
        ...
        ferramentas_mcp = [
            ...
            },
            {
                ...
                "function": {
                    ...
                    "description": "Busca itens similares (use quando cliente não encontrou o que quer)",
                    "parameters": {
                        "type": "object",
                        "properties": {
    ...
    async def _executar_ferramenta(self, nome: str, args: Dict, numero_cliente: str) -> str:
        ...
        # FERRAMENTAS LOCAIS
        if nome == "lista_itens":
            itens = listar_itens_disponiveis(self.itens_dir)
            return formatar_lista_para_mensagem(itens)

        elif nome == "consulta_faq":
            item_ativo = obter_item_ativo(numero_cliente, self.redis)
            if not item_ativo:
                return "ERRO: Cliente não tem item ativo."

            pergunta = args.get('pergunta', '')
            resultado_faq = consultar_faq_item(item_ativo, pergunta, self.itens_dir)
            ...
            resultado_tag = taguear_cliente(numero_cliente, item_id, self.redis, config['chatwoot'])
            if resultado_tag['sucesso']:
                return f"✅ Cliente marcado como interessado em {item_id}"
            else:
        ...
        # FERRAMENTAS MCP
        elif nome in ["analisar_sentimento", "gerar_proposta_comercial", "buscar_itens_similares",
...
if __name__ == "__main__":
    print("🧪 RAG Híbrido - Template Genérico\n")
